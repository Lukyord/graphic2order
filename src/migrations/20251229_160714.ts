...
export async function up({ db }: MigrateUpArgs): Promise<void> {
    await db.run(
        sql`ALTER TABLE \`work\` ADD \`list_hover_media_id\` integer REFERENCES media(id);`,
    )
    await db.run(
        sql`CREATE INDEX \`work_list_hover_media_idx\` ON \`work\` (\`list_hover_media_id\`);`,
    )
}
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  `)
    await db.run(
        sql`INSERT INTO \`__new_work\`("id", "orientation", "date", "main_media_id", "updated_at", "created_at") SELECT "id", "orientation", "date", "main_media_id", "updated_at", "created_at" FROM \`work\`;`,
    )
